          ...
          {navigation.map((item) => {
            ...
            return (
              <Link key={item.name} href={item.disabled ? "#" : item.href}>
                ...
                >
                  <item.icon className={cn("w-5 h-5 transition-colors", isActive ? "text-white" : "text-slate-300 group-hover:text-primary")} />
                  {item.name}
                </div>
              </Link>
            );
      ...
      <div className="p-6 border-t border-slate-50">
        <div className="bg-slate-50 rounded-3xl p-4 flex items-center gap-4 border border-slate-100">
           ...
           <div className="flex-1 min-w-0">
              <p className="text-sm font-black truncate text-slate-900">Officer 01</p>
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Chin Hin HQ</p>
           </div>
        </div>
      </div>
    </div>
      ...
      <div className="flex-1 lg:ml-80 flex flex-col min-h-screen">
        <header className="h-20 border-b border-slate-100 bg-white/80 backdrop-blur-xl flex items-center justify-between px-10 sticky top-0 z-40">
          <div className="flex items-center gap-4">
            <Button variant="ghost" size="icon" className="lg:hidden h-12 w-12 rounded-2xl bg-slate-50" onClick={() => setIsMobileOpen(true)}>
          ...
          <div className="flex items-center gap-6">
            <div className="hidden md:flex items-center gap-2 bg-emerald-50 px-4 py-2 rounded-2xl border border-emerald-100">
               ...
               <span className="text-[10px] font-black text-emerald-700 uppercase tracking-widest">Agent Orchestration Active</span>
            </div>
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="flex items-center gap-3 pl-3 pr-2 h-14 hover:bg-slate-50 rounded-2xl transition-all">
              ...
              <DropdownMenuContent align="end" className="w-64 p-3 rounded-3xl border-slate-100 shadow-2xl mt-2">
                <DropdownMenuLabel className="font-black text-[10px] text-slate-400 uppercase tracking-widest mb-2 px-2">Account Control</DropdownMenuLabel>
                <DropdownMenuItem className="rounded-2xl font-bold p-3 hover:bg-slate-50 cursor-pointer">Profile Settings</DropdownMenuItem>
                <DropdownMenuItem className="rounded-2xl font-bold p-3 hover:bg-slate-50 cursor-pointer">Team Management</DropdownMenuItem>
